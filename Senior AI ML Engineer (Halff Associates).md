# Professional Experience | Halff Associates

## AI Solutions Technical Lead
**Company:** Halff Associates  
**Role:** AI Solutions Technical Lead  
...
All civil engineering consulting and design services delivered by Halff require contractors to submit competitive bids to execute field construction work. Contractors quote prices based on the specific materials, processes, and quantities outlined by Halff (e.g., *20 ft of PVC pipe*, *fire hydrant installation*, *manhole excavation*). Halff engineers then aggregate these disparate estimates into a formalized bid sheet matrix where rows represent items and columns represent competing contractors. 

Historically, this aggregation was performed manually by individual engineers with zero structural oversight, resulting in highly inconsistent document layouts, fractured naming conventions, and unstandardized material descriptions (e.g., the exact same product could be named entirely differently across two sheets). The objective was to eliminate this fragmentation by creating a centralized, clean data repository mapping standardized item names, historical contractor pricing, and macroeconomic price fluctuations over time to enable rapid, data-driven bid generation.
...
3. **Zero-Shot Semantic Classification:** Designed and trained a zero-shot text classification model using Hugging Face transformers. This mapped arbitrary, unstructured contractor descriptions to Halff’s newly established master data dictionary of standard utility/resource categories.
...
##### 💻 Tech Stack
`Python` • `Hugging Face Transformers` • `PyTorch` • `SpaCy` • `Pandas` • `OCR Tools`

---

#### 2. Utility Pipe Failure Prediction (Predictive Analytics & Survival Analysis)
*Designed and deployed an end-to-end Machine Learning system combining physical asset telemetry, environmental variables, and historical fault data to forecast structural failures across utility networks.*

##### ⚙️ The Challenge & Business Context
Subsurface municipal utility infrastructure is notoriously expensive to survey and maintain. Traditional maintenance strategy relies heavily on reactive fixes or arbitrary schedule-based testing, which misallocates finite engineering budgets. The goal was to build an intelligent diagnostic model capable of prioritizing high-risk pipe segments, allowing municipalities to preemptively address structural degradation and mitigate catastrophic bursts.
...
4. **Gradient Boosting Regression:** Trained and tuned a `LightGBM` regression framework optimized to output continuous temporal windows indicating precisely when a pipe segment's next structural integrity breach would occur.

##### 📈 Impact & Business Value
* **Financial Savings:** Saved approximately **$168,000** in localized utility maintenance and diagnostic costs by optimizing targeted excavation schedules.
...
##### 🛠️ Architectural Implementation
1. **Sensor-to-Image Data Pipeline:** Programmatically queried the Google Street View API utilizing precise latitude and longitude coordinates derived from Halff's terrestrial and aerial LiDAR dart scanning payloads.
...
##### 📈 Impact & Business Value
* **60% Surveying Cost Reduction:** Slashed municipal field surveying expenditures by 60% through automated, cloud-scale inferences injected directly into active municipal databases.
...
| :--- | :--- |
| **Machine Learning / Deep Learning** | PyTorch, LightGBM, Scikit-Learn, Scikit-Survival, Hugging Face Transformers |
| **Natural Language Processing (NLP)**| SpaCy, Zero-Shot Transformers, Text Tokenization, Regex Data Extraction |
| **Data Engineering / Analytics** | Pandas, NumPy, Tabular Normalization, Spatial Feature Extraction |
| **Data Ingestion & APIs** | Google Street View API, OCR Engines, Geospatial LiDAR Coordinates |
